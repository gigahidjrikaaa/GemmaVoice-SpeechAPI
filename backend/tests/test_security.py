...
def test_require_api_key_noop_when_disabled() -> None:
    ...
    settings = Settings(_env_file=None, api_key_enabled=False)  # type: ignore[call-arg]
    require_api_key(request, settings=settings)  # Should not raise


def test_require_api_key_rejects_missing_header() -> None:
    request = _build_request({})
    settings = Settings(_env_file=None, api_key_enabled=True, api_keys=["secret"])  # type: ignore[call-arg]
